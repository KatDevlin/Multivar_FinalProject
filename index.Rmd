---
title: "Multivariate Stats Final Project"
author: "Kat Devlin"
date: "2017年12月11日"
...
Note 1: Analysis scripts are in a separate file.  
Note 2: I used to work at the Pew Research Center, including when this data was initially fielded, cleaned, etc. However, to the best of my knowledge, no one ever performed any of the analysis in this project while I was employed (or after), and at the time I left all analysis and data management was performed with Stata, SPSS or Excel. tl;dr: this is all original work. 


# Get the Data

The original file is an SPSS dataset from Pew Research Center's Global Attitudes project. Anyone can access the .sav file and associated codebook files for download from this site: http://www.pewglobal.org/datasets/2015/. You must create an account (free) to download the data so `download.file` command not appropriate here. I downloaded "Spring 2015 Survey Data" files, saved them in my working directory and now convert .sav to .csv. I have these lines commented out partially since the files are very large (too large for normal Github storage) and I don't want to keep writing/overwriting.

```{r}
# library(foreign)
# x <- read.spss("PewGlobal2015.sav", to.data.frame=TRUE)  # large file: 45K+ obs, 927 vars
# write.csv(x, file = "PewGlobal2015.csv")
# x <- read.csv("PewGlobal2015.csv", as.is = TRUE)  # After initial download use this
# table(x$COUNTRY)  # country names and n size to check everything worked
```
...
# t <- match(c("COUNTRY", "Q1", "Q3", "Q4", "Q6", "Q12A", "Q12B", "Q12C", "Q12D", "Q13A", "Q13B", "Q13E", "Q13F", "Q13G", "Q17", "Q18", "Q19B", "Q25A", "Q25D", "Q26A", "Q26B", "Q26C", "Q26D", "Q26E", "Q26F", "Q26G", "Q32", "Q33", "Q39", "Q40", "Q41", "Q42", "Q43", "Q44", "Q70", "Q71",  "Q152", "Q164", "WEIGHT"),names(x))
# Removed Q72, Q73, Q74

# Subset and write a new .csv to avoid loading the larger file each
...
# Use codebook (in initial Pew website download) for full question info
...
With the Pew Research individual-level data now in usable form, we need to 1) create a country-level dataset to make cross-country comparisons and 2) find and merge this data with some World Bank indicators we think will be interesting.

After much discussion with our stats professor, my partner and I settled on not using the survey weights and using country means to present the data at the country level. For this we had to mark as `NA` any "Don't know" or "Refused" responses to the questions, which knocked out a lot of respondents but still left us with n > 15K. Not the original plan - I had the weights syntax ready to go and originally recoded differently - but our professor told us to skip all that and just focus on the multivariate analysis part of the project without worrying about the DKs or trying to impute.
...
# Find the World Bank Data
```{r}
# Next we took data from the World Bank and will merge with country-level Pew data
# The data is available through the World Bank's very handy WDI library
